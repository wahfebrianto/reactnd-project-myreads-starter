...
test('show right title and amount of book', async () => {
    const { container } = render(<BookShelf shelfTitle={shelfTitle} updateBookLists={() => { }} books={books} />)
    expect(container.querySelector('.bookshelf-title').innerHTML).toBe(shelfTitle)
    expect(container.querySelectorAll('.books-grid li')).toHaveLength(books.length)
})