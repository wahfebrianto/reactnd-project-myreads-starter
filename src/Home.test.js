...
const bookShelvesCount = 3
const title = 'MyReads'

test('show title', () => {
    const { container } = render(<BrowserRouter><Home bookLists={bookLists} updateBookLists={() => { }} /></BrowserRouter>)
    expect(container.querySelector('.list-books-title>h1').innerHTML).toBe(title)
})

test('show three bookshelves', () => {
    const { container } = render(<BrowserRouter><Home bookLists={bookLists} updateBookLists={() => { }} /></BrowserRouter>)
    expect(container.querySelectorAll('.bookshelf')).toHaveLength(bookShelvesCount)
})

test('show one link button with /search as a target', () => {
    const { container } = render(<BrowserRouter><Home bookLists={bookLists} updateBookLists={() => { }} /></BrowserRouter>)
    expect(container.querySelector('.open-search a').getAttribute('href')).toBe('/search')
})

bookLists = []

test('not error when bookLists is empty', () => {
    render(<BrowserRouter><Home bookLists={bookLists} updateBookLists={() => { }} /></BrowserRouter>)
})